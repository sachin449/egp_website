...
const Marc = () => {
    ...
    return (
        <div className="h-screen relative flex flex-col md:flex-row mb-18 ml-10">
            {/* Main Heading */}
            <div className="absolute top-10 left-4 md:left-14 text-2xl md:text-4xl font-bold text-gray-800 z-20">
                Market
            </div>
 
            {/* Text Carousel */}
            <div className="text-background-container w-full md:w-[42%] h-1/2 md:h-screen bg-gray-300 z-10 overflow-hidden" ref={textScrollRef}>
                <div className="relative w-full h-full flex" style={{ transform: `translateX(-${currentIndex * 100}%)`, transition: 'transform 1s ease-in-out' }}>
                    {textSections.map((section) => (
                        <div key={section.id} className="w-full h-full flex-shrink-0">
                            <div className="p-4 md:p-8 bg-transparent z-10 flex-shrink-0">
                                <h2 className="subheading text-base md:text-lg mt-10 md:mt-20 mb-4 md:mb-10 mr-4 md:mr-10 text-balance">{section.subheading}</h2>
                                <h1 className="heading text-xl md:text-3xl mb-8 md:mb-16 mr-8 md:mr-20 text-balance">{section.heading}</h1>
                                <p className="paragraph text-sm md:text-lg mr-4 md:mr-20 font-normal text-balance">
                                    {section.paragraph}
                                </p>
                                <h1 className="mt-4 md:mt-8 text-blue-600 text-highlight transition duration-300 ease-in-out transform hover:text-black hover:translate-x-1">{section.diffsection}</h1>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {/* Image Carousel */}
            <div className="image-container absolute md:relative top-1/2 md:top-20 right-4 md:right-[100px] w-full md:w-[54%] h-[70vh] bg-white z-20 overflow-hidden" ref={imageScrollRef}>
                <div className="relative w-full h-full flex" style={{ transform: `translateX(-${currentIndex * 100}%)`, transition: 'transform 1s ease-in-out' }}>
                    {imageSections.map((image, index) => (
                        <div key={index} className="w-full h-full flex-shrink-0">
                            <img src={image.src} alt={image.alt} className="w-full h-full object-cover" onError={() => console.error(`Error loading image ${image.src}`)} />
                ...
                <div className="absolute bottom-4 w-full flex justify-center space-x-4">
                    <button onClick={() => handleManualScroll('left')} className="bg-gray-300 text-black p-2 md:p-3 rounded-full shadow-lg hover:bg-gray-400 transition-colors duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="w-4 md:w-6 h-4 md:h-6">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" />
                        </svg>
                    </button>
                    <button onClick={() => handleManualScroll('right')} className="bg-gray-300 text-black p-2 md:p-3 rounded-full shadow-lg hover:bg-gray-400 transition-colors duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="w-4 md:w-6 h-4 md:h-6">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
                        </svg>
                    </button>
